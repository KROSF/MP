...
Viajes *initViajes(int *n) {
  ...
  (*n) = 0;
  Viajes *tmp = NULL;
  char *id_viaje, *id_mat, *f_inic, *h_inic, *h_fin, *p_libres, *viaje,
      *importe, *estado;
  ...
  while (!feof(file)) {
    ...
    if (id_viaje == NULL || id_mat == NULL || f_inic == NULL ||
        h_inic == NULL || h_fin == NULL || p_libres == NULL || viaje == NULL ||
        importe == NULL || estado == NULL)
      exit(1);
...
Pasos *initPasos(int *n) {
  ...
  while (!feof(file)) {
    ...
    if (id_viaje == NULL || poblacion == NULL)
      exit(0);
...
void actualizarViajes(vViajes *v) {
  ...
  struct tm h_fin, f_inic,h_inic, hoy = *localtime(&t_hoy);
  ...
  for (int i = 0; i < v->tam_v; ++i) {
    if (v->viajes[i].Estado == 1) {
      ...
      if (fechaMenor(f_inic.tm_mday, f_inic.tm_mon, f_inic.tm_year)) {
        v->viajes[i].Estado = 3;
      } else if ((f_inic.tm_mday == hoy.tm_mday) && (f_inic.tm_mon == hoy.tm_mon + 1) &&
                 (f_inic.tm_year == hoy.tm_year + 1900)) {
        if(((h_inic.tm_hour * 60) + h_inic.tm_min) < ((hoy.tm_hour * 60) + hoy.tm_min))
        {
          v->viajes[i].Estado = 2;
        }
        else if (((h_fin.tm_hour * 60) + h_fin.tm_min) < ((hoy.tm_hour * 60) + hoy.tm_min)) {
          v->viajes[i].Estado = 3;
        }
      }
    }
