...
\node [test,join](ccc){logincount $<$ 3};
\node [proc,join](frlp){i = 0};
\node [proc,left= of frlp](frlp2){logincount++};
\node [test,below of=frlp,yshift= -1.5cm](con){i $<$ v$\rightarrow$tam\_v};
\node [test,join](con2){Usuario y Password correctos.};
\node [proc,right= of con2](cont){++i;};
\node [proc,right= of cont](rt1){return -1;};
\node [proc,below of=con2,yshift= -1.5cm](rt){return i;};
\node [term,join=by cong](fin2){fin};

\node [coord, left=of con] (c1)  {};

 \draw [->,lcnorm] (frlp.south) -- (con);
 \path (ccc.south) to node [near start,xshift=1em] {$si$} (frlp);
\path (con.west) to node [near start,yshift=1em] {$no$} (c1);
\draw [->,lcnorm] (con.west) -| (frlp2) |-(ccc);
\draw [->,lccong] (ccc.east) -| (rt1) |-(fin2);
